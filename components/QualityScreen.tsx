...
const SpeechBubble: React.FC<SpeechBubbleProps> = ({ x, y, lineStartX, lineStartY, lineEndX, lineEndY, visible, text, width = 500, delay = '0ms' }) => {
    ...
    >
      ...
      >
        <path
          ...
          fill="none"
        />
        <circle cx={lineEndX - svgLeft} cy={lineEndY - svgTop} r="7.5" fill="#0078FF" />
      </svg>
    </div>
  );
};
...
const QualityScreen: React.FC<QualityScreenProps> = ({ setPage, registerNextAction, registerPrevAction, setAutoRollDelay }) => {
  ...
  // Update Auto Roll Delay based on current state
  useEffect(() => {
    // Import dynamically or assume it's available. To avoid circular deps if needed, 
    // but here we just import the helper we made.
    const delay = getSlideDuration(Page.Quality, currentSlide, currentSlide === 1 ? slide2Step : undefined);
    setAutoRollDelay(delay);
  }, [currentSlide, slide2Step, setAutoRollDelay]);

  // Auto-advance Slide 2 Step 0 -> 1 (Request: Blue pins appear -> 1s wait -> Step 1)
  useEffect(() => {
    if (currentSlide === 1 && slide2Step === 0) {
      // Pins appear from 1000ms to ~2650ms. Wait 1s after (~3650ms). Rounded to 4000ms for safety.
      // This internal transition runs in parallel with the global timer.
      // We set global timer to a very high value for this specific step in config to avoid conflict.
...
const Slide2: React.FC<Slide2Props> = ({ isActive, step }) => {
  ...
  const getPinState = (key: string) => {
    ...
    // Step 1: Group 2 Active (Navigation) - Activates First
    else if (step === 1) {
      if (!isGroup1) {
    ...
    // Step 2: Group 1 Active (Infotainment) - Activates Second
    else if (step === 2) {
      if (isGroup1) {
        imageSrc = getAssetUrl(`/images/local-${key}.svg`);
        isLocalPin = true;
    ...
    >
      ...
      {Object.entries(pinPositions)
        ...
        .map(([key, pos]) => {
          ...
          const localPinWidth = key === 'pin6' ? 200 : 128;
          ...
          return (
            <div
              ...
              style={{
                ...
                zIndex: Math.round(1000 - pos.bottom) + (isLocalPin ? 100 : 0),
                opacity: isActive ? 1 : 0,
                transform: isActive ? 'translateY(0)' : 'translateY(40px)',
                transition: 'left 700ms ease-out, opacity 700ms ease-out, transform 700ms ease-out',
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            transform: step === 1 && isActive ? 'translateX(0)' : 'translateX(-1000px)',
            transitionDelay: step === 1 && isActive ? '500ms' : '0ms',
          }}
        >
          {/* 1번 - Badge */}
          <span
        ...
        <div
          className="absolute top-0 left-0 flex flex-col transition-all duration-700 ease-out"
          style={{
            width: '608px',
            opacity: step === 2 && isActive ? 1 : 0,
            transform: step === 2 && isActive ? 'translateX(0)' : 'translateX(-1000px)',
            transitionDelay: step === 2 && isActive ? '500ms' : '0ms',
          }}
        >
          {/* 1번 - Badge */}
          <span
...
const Slide5: React.FC<Slide5Props> = ({ isActive }) => {
  ...
  // Blue pins (index 0-4): appear AFTER black pins (start 5000ms+)
  // Black pins (index 5-16): appear FIRST (start 800ms+)
  const pinDelays = [
    4000, 4400, 4800, 5200, 5600, // Blue pins (Pin 1-5, indices 0-4) - FAST SPEED UP
    600, 900, 0, 1200, 1500, 0, 1800, 0, 0, 2100, 2400, 2700 // Black pins (Pin 6-17, indices 5-16) - FAST SPEED UP
  ];

  const [highlightPins, setHighlightPins] = React.useState(false);
  ...
  const handlePinHover = (name: string, cx: number, cy: number) => {
    const SVG_LEFT = 230;
    const SVG_BOTTOM = 80;
    const SVG_WIDTH = 1490;
    const SVG_VIEWBOX_WIDTH = 1495;
    const SVG_VIEWBOX_HEIGHT = 541;

    const scale = SVG_WIDTH / SVG_VIEWBOX_WIDTH;
    const svgHeight = SVG_VIEWBOX_HEIGHT * scale;
    // Assuming container is 1080px tall and SVG bottom is at 80px
    const svgTop = 1080 - SVG_BOTTOM - svgHeight;

    const screenX = SVG_LEFT + cx * scale;
    const screenY = svgTop + cy * scale;

    setTooltip({ text: name, x: screenX, y: screenY });
  };
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .pin-breathe {
          animation: pinBreathe 2.5s ease-in-out infinite;
        }
      `}</style>
      ...
      >
        ...
        {/* Blue Pin 1 - Mexico */}
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[0]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 1', 158, 219)}
          onMouseLeave={() => setTooltip(null)}
        >
          <g className={highlightPins ? 'pin-breathe' : ''} style={{ animationDelay: '0s' }}>
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M158.254 388C138.636 388 124.234 381.526 124.004 372.62C123.911 369.503 125.619 366.557 128.896 364.056C134.759 359.638 145.514 357 157.746 357C177.318 357 191.719 363.44 191.996 372.346C192.089 375.463 190.427 378.409 187.15 380.909C181.287 385.328 170.532 388 158.3 388H158.254Z" fill="#000000" />
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M157.661 379C145.255 379 136.148 375.032 136.002 369.573C135.944 367.663 137.024 365.857 139.096 364.325C142.803 361.617 149.604 360 157.339 360C169.716 360 178.823 363.947 178.998 369.406C179.056 371.316 178.005 373.122 175.933 374.654C172.226 377.362 165.425 379 157.69 379H157.661Z" fill="#000000" />
            <path className="transition-colors duration-1000" fillRule="evenodd" clipRule="evenodd" d="M166 367C166.046 369.209 162.285 371 157.585 371C152.884 371 149.047 369.209 149 367C148.954 364.791 152.73 363 157.415 363C162.1 363 165.953 364.791 166 367Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            <path className="transition-colors duration-1000" d="M129 273H187L160.053 368H156.061L129 273Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            ...
            <circle className="transition-colors duration-1000" cx="158" cy="219" r="60" stroke={highlightPins ? "#005FF9" : "#000000"} strokeWidth="8" />
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[1]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 2', 416, 325)}
          onMouseLeave={() => setTooltip(null)}
        >
          <g className={highlightPins ? 'pin-breathe' : ''} style={{ animationDelay: '0.4s' }}>
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M417.254 496C397.636 496 383.234 489.526 383.004 480.62C382.911 477.503 384.619 474.557 387.896 472.056C393.759 467.638 404.514 465 416.746 465C436.318 465 450.719 471.44 450.996 480.346C451.089 483.463 449.427 486.409 446.15 488.909C440.287 493.328 429.532 496 417.3 496H417.254Z" fill="#000000" />
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M417.661 488C405.255 488 396.148 483.614 396.002 477.581C395.944 475.47 397.024 473.474 399.096 471.78C402.803 468.787 409.604 467 417.339 467C429.716 467 438.823 471.362 438.998 477.396C439.056 479.507 438.005 481.503 435.933 483.197C432.226 486.19 425.425 488 417.69 488H417.661Z" fill="#000000" />
            <path className="transition-colors duration-1000" fillRule="evenodd" clipRule="evenodd" d="M425 474C425.046 476.209 421.285 478 416.585 478C411.884 478 408.047 476.209 408 474C407.954 471.791 411.73 470 416.415 470C421.1 470 424.953 471.791 425 474Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            <path className="transition-colors duration-1000" d="M387 380.5H445L418.053 474H414.061L387 380.5Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            ...
            <circle className="transition-colors duration-1000" cx="416" cy="325" r="60" stroke={highlightPins ? "#005FF9" : "#000000"} strokeWidth="8" />
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[2]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 3', 1358, 144)}
          onMouseLeave={() => setTooltip(null)}
        >
          <g className={highlightPins ? 'pin-breathe' : ''} style={{ animationDelay: '0.8s' }}>
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M1359.25 315C1339.64 315 1325.23 308.526 1325 299.62C1324.91 296.503 1326.62 293.557 1329.9 291.056C1335.76 286.638 1346.51 284 1358.75 284C1378.32 284 1392.72 290.44 1393 299.346C1393.09 302.463 1391.43 305.409 1388.15 307.909C1382.29 312.328 1371.53 315 1359.3 315H1359.25Z" fill="#000000" />
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M1358.66 307C1346.26 307 1337.15 302.614 1337 296.581C1336.94 294.47 1338.02 292.474 1340.1 290.78C1343.8 287.787 1350.6 286 1358.34 286C1370.72 286 1379.82 290.362 1380 296.396C1380.06 298.507 1379.01 300.503 1376.93 302.197C1373.23 305.19 1366.42 307 1358.69 307H1358.66Z" fill="#000000" />
            <path className="transition-colors duration-1000" fillRule="evenodd" clipRule="evenodd" d="M1366 293C1366.05 295.209 1362.29 297 1357.58 297C1352.88 297 1349.05 295.209 1349 293C1348.95 290.791 1352.73 289 1357.42 289C1362.1 289 1365.95 290.791 1366 293Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            <path className="transition-colors duration-1000" d="M1329 199.5H1387L1360.05 293H1356.06L1329 199.5Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            ...
            <circle className="transition-colors duration-1000" cx="1358" cy="144" r="60" stroke={highlightPins ? "#005FF9" : "#000000"} strokeWidth="8" />
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[3]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 4', 1117, 209)}
          onMouseLeave={() => setTooltip(null)}
        >
          <g className={highlightPins ? 'pin-breathe' : ''} style={{ animationDelay: '1.2s' }}>
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M1118.25 380C1098.64 380 1084.23 373.526 1084 364.62C1083.91 361.503 1085.62 358.557 1088.9 356.056C1094.76 351.638 1105.51 349 1117.75 349C1137.32 349 1151.72 355.44 1152 364.346C1152.09 367.463 1150.43 370.409 1147.15 372.909C1141.29 377.328 1130.53 380 1118.3 380H1118.25Z" fill="#000000" />
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M1117.66 372C1105.26 372 1096.15 367.614 1096 361.581C1095.94 359.47 1097.02 357.474 1099.1 355.78C1102.8 352.787 1109.6 351 1117.34 351C1129.72 351 1138.82 355.362 1139 361.396C1139.06 363.507 1138.01 365.503 1135.93 367.197C1132.23 370.19 1125.42 372 1117.69 372H1117.66Z" fill="#000000" />
            <path className="transition-colors duration-1000" fillRule="evenodd" clipRule="evenodd" d="M1125 358C1125.05 360.209 1121.29 362 1116.58 362C1111.88 362 1108.05 360.209 1108 358C1107.95 355.791 1111.73 354 1116.42 354C1121.1 354 1124.95 355.791 1125 358Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            <path className="transition-colors duration-1000" d="M1088 263H1146L1119.05 358H1115.06L1088 263Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            ...
            <circle className="transition-colors duration-1000" cx="1118" cy="208" r="60" stroke={highlightPins ? "#005FF9" : "#000000"} strokeWidth="8" />
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[4]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 5', 938, 112)}
          onMouseLeave={() => setTooltip(null)}
        >
          <g className={highlightPins ? 'pin-breathe' : ''} style={{ animationDelay: '1.6s' }}>
            ...
            <path opacity="0.2" fillRule="evenodd" clipRule="evenodd" d="M939.661 275C927.255 275 918.148 270.614 918.002 264.581C917.944 262.47 919.024 260.474 921.096 258.78C924.803 255.787 931.604 254 939.339 254C951.716 254 960.823 258.362 960.998 264.396C961.056 266.507 960.005 268.503 957.933 270.197C954.226 273.19 947.425 275 939.69 275H939.661Z" fill="#000000" />
            <path className="transition-colors duration-1000" fillRule="evenodd" clipRule="evenodd" d="M947 261C947.046 263.209 943.285 265 938.585 265C933.884 265 930.047 263.209 930 261C929.954 258.791 933.73 257 938.415 257C943.1 257 946.953 258.791 947 261Z" fill={highlightPins ? "#005FF9" : "#000000"} />
            <path className="transition-colors duration-1000" d="M909 167.5H967L940.052 261H936.06L909 167.5Z" fill={highlightPins ? "#005FF9" : "#000000"} />
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[5]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 6', 32.5088, 245.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[6]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 7', 359.509, 479.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[8]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 9', 943.509, 343.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[9]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 10', 1250.51, 317.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[11]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 12', 787.509, 120.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[14]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 15', 1279.51, 229.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[15]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 16', 1461.51, 435.509)}
          onMouseLeave={() => setTooltip(null)}
        >
        ...
        <g
          className={`pin-animate ${isActive ? 'active' : ''}`}
          style={{ animationDelay: `${pinDelays[16]}ms`, cursor: 'pointer' }}
          onMouseEnter={() => handlePinHover('Pin 17', 1432.51, 245.509)}
          onMouseLeave={() => setTooltip(null)}
        >
      ...
      <img
        ...
        style={{
          ...
          left: '810px',
          zIndex: 50,
          transitionDelay: isActive ? '8000ms' : '0ms',
        }}
      />

      {/* Tooltip */}
      <div
        className={`absolute z-50 pointer-events-none transition-opacity duration-200 ${tooltip ? 'opacity-100' : 'opacity-0'}`}
        style={{
          left: tooltip ? `${tooltip.x}px` : 0,
          top: tooltip ? `${tooltip.y}px` : 0,
          transform: 'translate(-50%, -120%)', // Centered horizontally, moved up
        }}
      >
        <div
          className="relative px-4 py-2 bg-black/80 backdrop-blur-md rounded-lg shadow-xl border border-white/10"
        >
          <span className="text-white text-lg font-bold font-['42dot_Sans'] whitespace-nowrap">
            {tooltip?.text}
          </span>
          {/* Arrow */}
          <div
            className="absolute left-1/2 -bottom-2 w-4 h-4 bg-black/80 backdrop-blur-md border-r border-b border-white/10 transform -translate-x-1/2 rotate-45"
          ></div>
        </div>
      </div>
